...
describe('AttachmentFilesBase64', () => {
  describe('addAttachmentFilesAsync', () => {
    ...
    it.skip('should add attachment files to tests', async () => {
      const result = await AttachmentFilesBase64.addAttachmentFilesAsync([mockTest], mockFilePaths);

      expect(result[0].attachmentFiles).toHaveLength(1); // Assuming only one of the attachments is an image
      expect(result[0].attachmentFiles![0].fileName).toEqual('20240312_051645_screenshot.png');
    ...
    it('should handle error when attachment file not found', async () => {
      ...
      if (mockTestWithError.gherkinLogs) {
        mockTestWithError.gherkinLogs[1].attachments = ['path/to/attachment2.txt'];
      }

      await AttachmentFilesBase64.addAttachmentFilesAsync([mockTestWithError], mockFilePaths);
